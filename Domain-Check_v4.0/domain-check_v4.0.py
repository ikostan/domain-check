...
try:
    ...
    logging.debug('Logging config error')
    logging.info('Logging config error: ' + str(error.message))
...
def setConfigs(Config_file): #Read configurations file
    ...
    if isSilent == True:
        ...
        if sendEmail == True and ((userName== "" or userName== None) or (userPassword == "" or userPassword == None)): #Ask for username/password (in case when UNP values from config.csv are empty or null)
            print "\n=====================================================================================\n"         
...
'tech_email: ' + str(tech_email) + ' ' + 'error_code: ' + str(error_code) + ' ' + 'email: ' + str(email_) + ' ' + 'reseller_email: ' + str(reseller_email))
...
def write_csv(ipList_file, mailSentNum, isEmailAccountValid, isSent, test_sendEmail, test_emailAddress, registrant_email, registrar_abuse_contact_email, e_mail, admin_email, tech_email, error_code, email_, reseller_email):
    ...
    with codecs.open(ipList_file,'a','utf-8-sig') as csvFile2:      #Open file
        ...
        for row in domains:                                         #start loop            
            ...
            if isValid == 0:
                ...
                writer.writerow([domains[i], domainIP, 'invalid', emails[0], emails[1], emails[2], emails[3], emails[4], str(Admin_Name), 'not implemented', str(isSent), str(reason), recipient_email, timeNow])
                #logs
                logging.info('Send email notification desicion: END')
                logging.debug(str(domains[i]) + " " + str(domainIP) + " " + 'invalid' + " " + str(emails[0]) + " " + str(emails[1]) + " " + str(emails[2]) + " " + str(emails[3]) + " " + str(emails[4]) + " " + str(Admin_Name) + "address: n/a" + " False " + str(reason) + " " + str(recipient_email) + " " + str(timeNow))     
                ...
                writer.writerow([domains[i], domainIP, 'valid', 'n/a', 'n/a', 'n/a', 'n/a', 'n/a', 'n/a', 'n/a','False','Valid Domain Address', recipient_email, timeNow])  
                #logs
                logging.info('Writing:[domains[i], domainIP, valid, n/a, n/a, n/a, n/a, n/a, n/a, n/a, False: Valid Domain Address, recipient_email, date/time')
                logging.debug(str(domains[i]) + " " + str(domainIP) + " valid, n/a, n/a, n/a, n/a, n/a, n/a, n/a, False: Valid Domain Address " + recipient_email + str(timeNow))
            print("\n")
...
invalidName, mailSentNum = write_csv(ipList_file, mailSentNum, isEmailAccountValid, isSent, test_sendEmail, test_emailAddress, registrant_email, registrar_abuse_contact_email, e_mail, admin_email, tech_email, error_code, email_, reseller_email) #call write_csv function
